...
\chapter{Isolating flow-dependent uncertainty in ensemble reanalysis data and
  its relation to Euro-Atlantic weather regimes and warm conveyor belts} % LaTeX chapter title
\label{ch:weatherRegimes}

\chapteroverview{The nature of assimilation uncertainty for a selected reanalysis,
    \gls{era5}, is examined. By leveraging a range of statistical
    techniques and uniquely combining a set of well-established datasets,
    the several influencing factors are isolated and their relative importance
    assessed. The development of the
...
The patterns discussed above remain clearly visible when accounting for the difference in (temporal) error variances between the different segments. We provide the appropriate measure in \cref{sec:EDA_AddFig} in \cref{fig:R2GLS}.
...
\begin{subappendices}
    ...
    \begin{figure}[!h]
        ...
        \caption{BIC and RSS for various segmentations. A minimum of the BIC corresponds to the preferred segmentation. Data and model assumption as described in \cref{sec:EDA_resbrk}. The resulting model and break points are shown in \cref{fig:Segs}.}
        \label{fig:BICSeg}
    \end{figure}
    ...
    \begin{figure}
        ...
        \caption{Same as \cref{fig:WREffect}, but marginal means estimated from the full models from \cref{sec:EDA_resmod} extended by the weather regime time series as a covariate.}
