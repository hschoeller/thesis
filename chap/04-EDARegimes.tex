...
As suggested from the patterns in \cref{fig:WREffect}, the largest variability in the \glspl{wr}' effect on the residual uncertainty is found in the eastern northern Atlantic. This is in contrast to the overall residual variance (cf.~\cref{fig:R2}), where the largest variability was found in the south western Atlantic, indicating the reason(s) for that maximum are not well represented by the \gls{wr} clustering. That is not surprising, since the \glspl{wr} are inherently designed for differentiating large scale structures, whereas it is sensible to assume, as we have seen, that the assimilation uncertainty is governed by small scale processes and observation specifics.

Additionally, the \gls{wr} specific \gls{zg500} patterns themselves exhibit a fairly large variability in the North Atlantic compared to the Gulf Stream region, as shown in \cref{fig:WRStd} in \cref{sec:EDA_AddFig}. This is because the \glspl{wr} partition the state space not along the axis of the largest variability in assimilation uncertainty of \gls{zg500}, but along the axis of largest variability in \gls{zg500} itself. Compared to the variability in \gls{wr} patterns, high values of $\sigma_{\mathrm{WR}}$ do not extend so much into the polar regions. Given the above considerations, we argue the spatial pattern of $\sigma_{\mathrm{WR}}^{\mathrm{(simple)}}$ can be understood as a combination of the variability in the response itself ($V_{\mathrm{res}}$) driven mainly by strom track dynamics and variability in the \gls{wr} fields themselves.

\begin{figure}
    \centering
    \includegraphics[scale=1]{fig/WCD/figComb/combFig2.pdf}
    \caption{Weather regime pattern variability as measured by the standard deviation of the \glspl{emm} across the 8 weather regime categories. (a) Standard deviation for the models with the weather regimes as the sole covariate. (b) Difference in standard deviation to a model with additional covariates introduced in the text relative to the standard deviation across the regimes for the simple model (a). Positive values indicate pattern attenuation. Values shown for grid points with \gls{fdr}-adjusted p-values below 5\%.}
    \label{fig:2ndRMS}
\end{figure}
...
\begin{figure}
    ...
    \caption{Relative weather regime pattern variability attenuation as measured by $\Delta \sigma_{\mathrm{WR}}$, but with one covariate removed from the full model~\eqref{eq:fullMod}. The removed covariate is indicated atop the facets. Values shown for grid points with \gls{fdr}-adjusted p-values below 5\%}
    \label{fig:loo}
\end{figure}
...
As outlined in the introduction, \glspl{wcb} are well established as amplifiers of forecast uncertainty, with much of that uncertainty commonly attributed to model formulation rather than to the initial state. Our setting differs in this respect: as seen in the previous section, the target variable---the variance-balanced residuals of the first-stage models $\hat{\varepsilon}^*$---is severely influenced by specifics of the observation system.
...
equally relevant, and we aim to contribute to the body of literature reviewed above by studying how \glspl{wcb} relate to assimilation uncertainty specifically.
...
elevated \gls{wcb} activity entails for each weather regime. Concretely, for
...
\begin{subappendices}
    ...
    We provide classical composites for comparison to \glspl{emm}. Define $\{I_{l,t}\}_{l=1}^L$, the binary indicators for the $L$ different levels
    \begin{equation}
        I_{l,t} =
        \begin{cases}
            1, & \text{if }x^{(c)}_t = l, \\
    ...
    \begin{figure}
        ...
        residuals $\hat{\varepsilon}^*$ and the \gls{zg500} anomalies ased on annual
        ...
        of the \gls{zg500} anomalies. Same quantity as on the right of
        \cref{fig:2ndRMS}. Values below or equal to $-1.5$ are shown in the
    ...
    \begin{figure}[H]
        ...
        % \includegraphics[scale=.75]{fig/WCD/f18.pdf}
...
\begin{figure}
    \centering
    \includegraphics[scale=.75]{fig/WCD/f23.pdf}
    \caption{Logarithmic \gls{eda} ensemble spread of daily geopotential at 500 hPa assimilated (triangles) and 95\% \glspl{ci} for the mean estimates (shadings) for the year 2024. The blue (orange) triangles and shading corresponds to the grid point indicated by an upward (downward) triangle in figure~\ref{fig:R2}.}
...
\end{figure}
    \begin{figure}
        ...
        \caption{Composite mean maps of the variance-balanced residuals $\hat{\varepsilon}^*$. Values
            shown for grid points with \gls{fdr}-adjusted p-values below 5\% derived from the block-permutation test described in \cref{sec:EDA_sig} with 10,000 draws. Green contours display the composite mean Z500 maps for each of the weather regimes based on the \gls{era5} HRES product.}
    ...
    \begin{figure}
        ...
        the weather regime time series as a covariate (shading). Shown for
    ...
    \begin{figure}
        ...
        \caption{\gls{emm} effects of each of the weather regimes on the (latitude-weighted) spatially averaged variance-balanced residuals $\hat{\varepsilon}^*$. Points indicate estimate and lines the 95\% \glspl{ci}. Confidence intervals excluding 0 imply statistical significance.}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=.75]{fig/WCD/figComb/Composite_corr_mapsSupp.pdf}
    ...
    \begin{figure}
        ...
        \caption{Standard deviation of \gls{zg500} across each of the eight weather regimes. The patterns are shown as dark green contours e.g. in \cref{fig:WREffect}.}
        \label{fig:WRStd}
    \end{figure}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=.75]{fig/WCD/figComb/composite_corr_maps_wcb.pdf}
