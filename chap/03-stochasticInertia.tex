...
% \raisebox{-.5\height}{\includegraphics[scale=.75]{fig/CAMCOS/CdVloglog.pdf}}
        ...
        Also indicated are the ``blocking'' fixed point (black) and the
        ``zonal'' fixed point (red). (b) Normalized sum of affinities according
        to \cref{eq:simiMat} plotted over the noise strength $\sigma$ for all
        points shown in (a) (``All'') as well as for points either in the
        blocking regime or within a distance of
        $3\sqrt{2\sigma_{\max}^2 \Delta t}$ of any point in the blocking
        regime, where $\sigma_{\max}=0.03$ (``Blocking''). The dashed lines
        show the tangent at $\sigma = \sigma^{\ast}$
        (indicated by open circles), the noise level maximizing the slope of
        the curve with the maximum slope~$d_\mathcal{X}$ (``All'')
        and $d_\mathcal{M}$ (``Blocking''). Both values are indicated for
...
\begin{figure}[!htbp]
    ...
    the CdV system. The hollow markers belong to the expected escape times
    ...
    \cref{fig:SetsDT1} and the ``blocking'' fixed point as a black cross.}
...
\begin{figure}[!htbp]
    ...
    ``blocking'' fixed point as a black cross. The black dashed line shows the
...
% \enlargethispage{2\baselineskip}
    \begin{figure}[!htbp]
        ...
        $10^{-5}$. We subsample the trajectories such that both methods have a
